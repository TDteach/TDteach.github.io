...
导读直接取自同次选题和写作产物，不再另行调用模型。上游程序验证论文身份、顺序、长度及正文 hash；模板只呈现已经绑定的纯文本字段。
...
`homepage.items` 的论文 ID 和顺序必须与所选正文一致。标题和理由来自已绑定文章的 headline 与 reading_reason；整期标题和导语来自同次选题产物。生成、绑定和本地核验由上游负责，页面只呈现已有结果。`article_index` 仍可供归档检索使用，保留原文标题等元数据，但不作为头部导读。链接格式是 `#paper-` 加论文 ID，仅将旧式 ID 中的 `/` 替换为 `-`。排版不修改原始 article 或 content_hash。
