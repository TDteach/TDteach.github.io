...
## Negative association bounds integer-order Rényi divergence by marginal-matched Poisson sampling under stated Gaussian conditions
...
Read this if you audit DP-SGD sampling: it supplies concrete checks—negative association and sign balance—before reusing an integer-order Poisson RDP bound, while marking two regimes where that reuse is not justified.

[Abstract; Theorem 3.2 and Proposition 3.4 (Section 3); Theorem 4.3 (Section 4)](https://arxiv.org/abs/2609.01944v1)

## Forward KD simultaneously improves reasoning and factual recall during pre-training relative to standard next-token prediction (NTP), but it instead slows factual recall acquisition during mid-training despite continued reasoning gains.
...
It gives a concrete entropy-routed KD experiment to test during mid-training, together with a measured reasoning–factual-recall tradeoff that uniform KD can obscure.
...
## Global granularity outperforms oracle layer repair in matched-budget 4-bit quantization tests
...
Read this paper to learn how to distinguish a layer that appears damaged from one whose precision restoration causally recovers accuracy, and to see a matched-budget test favoring global granularity over layer protection.

[Abstract; §4 and Appendix E; §5 (Table 1); Appendix F (8-bit tier, Table 8); §6 Limitations](https://arxiv.org/abs/2609.01587v1)

## Exact-state repair isolates memory formation as the bottleneck in EAL-Bench
...
Read this paper to borrow a concrete repair-and-replay protocol that distinguishes authorization errors written into agent memory from errors introduced by the executor.

[Sections 3.1, 4.1–4.4, 4.6; Tables 3, 6, 21, and 24](https://arxiv.org/abs/2609.01836v1)

## The rating decision crystallizes at a sharp, late layer that is shared across all attacks.
...
Use this paper to adapt a concrete controlled-corruption, activation-patching, and layerwise-decoding protocol for diagnosing how an LLM judge converts detected defects into scores, while preserving tests for missed perturbations.

[Abstract; §5.2; Figure 5; Tables 2–3; §7 Limitations](https://arxiv.org/abs/2609.01604v1)

Prepared retrospectively from the 2026-09-03 candidate papers; verified on 2026-09-12.
