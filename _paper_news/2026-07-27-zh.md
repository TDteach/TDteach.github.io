...
brief_title: 可靠性开始依赖结构。
brief_dek: 今天最强的一批论文都在强调可审计脚手架：确定性决策层、证据使用审计，以及受治理的记忆与验证循环，正比更流畅的生成更能带来可靠性。
lead_paper:
  ...
  why: 它清楚表明：在高风险决策里，受约束的决策层比自由生成式 LLM 建议更可靠。
  skepticism: 最强证据仍来自单一、结构化很强的决策场景，而非开放世界部署。
  tags:
  - 可靠性
  - 高风险
  - 确定性
  - 评估
takeaways:
- 高风险用例持续奖励受约束系统而非开放式生成：监督式决策层、符号工具和校准路由，反复优于原始 LLM 行为。
- 评估正在进入过程内部。今天最强的论文关注系统是否使用了正确证据、采用了正确策略，而不仅仅是是否抵达正确终点。
- 长时程 agent 越来越成败于状态治理——递归验证、记忆生命周期和受治理执行的重要性已经接近模型规模本身。
themes:
- title: 结构化可靠性
  text: 在高风险决策里，确定性策略与校准路由持续优于不受约束的 LLM 行为。
- title: 过程审计
  text: 今天最强的评估工作审查证据使用、澄清策略和采用动态，而不只看最终准确率。
- title: 受治理 agent
  text: 递归验证和记忆契约正成为长时程 agent 系统的基础设施。
signals:
- label: 部署落差
  title: 自由输出在高风险场景会失手。
  text: Deterministic Decisions、置信度路由和 Euclid-MCP 都通过把生成与执行或延迟分开来提升可信度。
- label: 评估转向
  title: 成功现在需要审计线索。
  text: 医学证据审计与基于遗憾的澄清评测表明，最终准确率会掩盖证据误用、过度干预和糟糕的停机策略。
- label: Agent 模式
  title: 验证循环正在前移。
  text: AREX、Mi-Memory 和 MemTools 都把递归检查与记忆契约当作核心 agent 基础设施，而非附加组件。
top_papers:
- title: Deterministic Decisions for High-Stakes AI. A Zero-Egress Pipeline with the Deployability of RAG and the Accuracy of Machine Learning
  url: https://arxiv.org/abs/2606.29280v1
  pdf: https://arxiv.org/pdf/2606.29280v1.pdf
  why: 如果你关心高后果场景中的选择性自动化，这篇论文最值得先读。
  why_now: 它给出了一个明确警告：在行动偏差重要的任务里，流畅的 RAG 输出并不值得直接信任。
  skepticism: 其主任务仍是高度结构化、特定领域的设定。
- title: Auditing Evidence Use in Medical LLM Diagnosis
  ...
  why: 它是很好的配套论文：说明没有忠实证据使用时，正确答案本身并不够。
  why_now: 过程级审计正从安全研究扩散到医疗等真实部署领域。
  skepticism: 解释仍部分依赖专家定义的证据角色与审查过程。
- title: 'AREX: Towards a Recursively Self-Improving Agent for Deep Research'
  ...
  why: 它把验证提升为研究轮次之间的递归控制信号，而不只是最后一步 rerank。
  why_now: 深度研究 agent 正在变成长时程系统，控制循环比单次答案更关键。
  skepticism: 基准提升还不能证明在更脏、更开放的真实研究任务上也能稳定自我改进。
- title: 'One More Turn, Less Regret: A Regret-Based Multi-Turn Benchmark for LLMs'' Clarification Policies'
  ...
  why: 如果你关心模型如何花费交互轮次，而不只是最终是否答对，这篇论文很有用。
  why_now: 澄清行为正在变成真实部署中的产品选择，直接影响成本与用户摩擦。
  skepticism: 像大多数对话策略基准一样，它仍依赖受控的隐藏意图设定。
- title: 'When to Trust the Map: Confidence-Aware LLM Routing for Automotive CVE-to-ATM Mapping'
  ...
  why: 它展示了一个具体的选择性自动化设计：用校准后的置信度决定 LLM 何时应该独自行动。
  why_now: 许多安全关键流水线真正需要的是路由与延迟，而不是二元化的“全自动幻想”。
  skepticism: 当前证据仍较强地局限于单一领域和单一主干模型族。
---
