...
brief_dek: "Today’s strongest papers shift safety from output filtering to runtime structure, trace-level auditing, and post-deployment checks, with quantization and memory emerging as major failure surfaces."
takeaways:
...
themes:
  - title: "Agent safety is moving from outputs to execution traces"
    text: "Several papers argue that final outputs are too coarse to assess real agent safety. Unsafe tool use, unauthorized access, and failure causes often occur mid-trajectory and remain invisible if evaluation only checks the last answer."
  - title: "Structural defenses are replacing prompt-only defenses for web and agent security"
    text: "The most credible defenses in this batch reduce attack surface by construction: separating planning from execution, enforcing runtime isolation, or binding memory/tool use to explicit authority. This is a stronger pattern than asking the model to “be careful.”"
  - title: "Prompt injection remains central, but defenses are diversifying"
    text: "Web and multimodal agents remain highly exposed to indirect prompt injection and jailbreaks. The notable change is that defenses now span architecture, compact guard models, and direct model editing rather than a single filtering layer."
lead_paper:
  title: "Auditing Agent Harness Safety"
  url: "https://arxiv.org/abs/2605.14271v1"
  pdf: "https://arxiv.org/pdf/2605.14271v1.pdf"
  why: "It makes the day’s core point concrete: agent safety must be measured over full execution traces, not final answers."
  skepticism: "It is strongest as an auditing lens; the paper is less definitive on which mitigations reliably fix the exposed failures."
  tags:
    - "agent-safety"
    - "evaluation"
    - "execution-traces"
...
signals:
  - label: "Signal"
    title: "Trace-level safety is becoming the default."
    text: "Auditing Agent Harness Safety, Holistic Evaluation and Failure Diagnosis, and least-privilege testing all show final-answer success misses unsafe intermediate actions."
  - label: "Tension"
    title: "Deployment transforms can erase safety gains."
    text: "Unlearning can fail after quantization, quantization can activate malicious behavior, and anti-malicious-finetuning defenses break under adaptive attackers."
  - label: "Bet"
    title: "Structure will beat prompt-only defenses."
    text: "Plan-then-execute, lineage-bound memory, OS-style isolation, and verifiable workflows all reduce attack surface by separating control from untrusted content."
top_papers:
  - title: "Auditing Agent Harness Safety"
    url: "https://arxiv.org/abs/2605.14271v1"
    pdf: "https://arxiv.org/pdf/2605.14271v1.pdf"
    why: "Useful if you deploy agents: it exposes permission and information-flow failures that output-only evaluation misses."
    why_now: "Teams are shipping tool-using and multi-agent systems without good visibility into mid-trajectory safety failures."
    skepticism: "It diagnoses failures well, but mitigation guidance is less developed than the auditing framework."
  - title: "Web Agents Should Adopt the Plan-Then-Execute Paradigm"
    ...
    why: "A strong architectural argument that web-agent security improves when planning is separated from untrusted page content."
    why_now: "Prompt injection remains a deployment blocker for web agents, and this offers a structural alternative to more filtering."
    skepticism: "Its practicality depends on trusted typed APIs or maintained SDKs, which many real sites still lack."
  - title: "Forgetting That Sticks: Quantization-Permanent Unlearning via Circuit Attribution"
    ...
    why: "Important because it shows safety work can be undone by a common deployment step and proposes a mechanistic fix."
    why_now: "Many release pipelines quantize models after alignment or unlearning, so pre-quantization safety claims are no longer enough."
    skepticism: "Evidence is strongest on smaller model scales and factual-recall settings, with added compute and method complexity."
  - title: "WARD: Adversarially Robust Defense of Web Agents Against Prompt Injections"
    ...
    why: "A deployment-oriented web-agent defense combining large-scale attack data, adaptive training, and efficient guard-model use."
    why_now: "Web-agent deployments need defenses that survive adaptive attacks, not just static prompt-injection filters."
    skepticism: "The paper explicitly leaves pixel-level and camouflaged task-aligned attacks as open failure modes."
  - title: "MemLineage: Lineage-Guided Enforcement for LLM Agent Memory"
    ...
    why: "Cryptographic provenance for agent memory directly targets persistent prompt-injection risks."
